...
cmdAddMorador:  .asciiz "addMorador"  # ln 672
cmdAddAuto:  .asciiz "addAuto"        # ln 150
cmdRmvMorador: .asciiz "rmvMorador"   # ln 587
cmdLimparAp:     .asciiz "limparAp"   # ln 310
cmdInfoAp:       .asciiz "infoAp"     # ln 498
cmdInfoGeral: .asciiz "infoGeral"     # ln 269
cmdSalvar:   .asciiz "salvar"         # ln 373
cmdRecarregar: .asciiz "recarregar"   # ln 421
cmdFormatar: .asciiz "formatar"       # ln 480
cmdExit:   .asciiz "exit"             # ln 140

numApto: .asciiz "AP: "
tipoInvalido: .asciiz "Opção inválida.\n"
msgAptoVazio: .asciiz "Apartamentos vazios."
msgMoradores: .asciiz "Moradores: \n"
comandoInvalido: .asciiz "Comando inválido.\n"
msgAptoCheio:  .asciiz "Apartamento cheio! Nao pode adicionar mais moradores.\n"
msgMoradorRegistrado: .asciiz "Morador cadastrado com sucesso!"
msgRegistrar:   .asciiz "Digite o nome do morador:\n"
msgAndar:       .asciiz "Digite o andar (1-12):\n"
msgApto:        .asciiz "Digite o numero do apartamento (1 ou 2):\n"
apInvalido: .asciiz "Número de apartamento inválido!\n"
msgVagasOcupadas: .asciiz "\nEsse apartamento ja esgotou todas as vagas disponiveis\n"
msgVeiculo:     .asciiz "\nVocê deseja registrar um carro ou moto? (1=carro, 2=moto):\n"
msgModelo:      .asciiz "\nDigite o modelo do veículo:\n"
msgPlaca:       .asciiz "\nDigite a placa do veículo:\n"
msgCarroRegistrado: .asciiz "\nCarro registrado com sucesso!\n"
msgMotoRegistrada:  .asciiz "\nMoto registrada com sucesso!\n"
msgCarroExistente:  .asciiz "\nEste apartamento não pode registrar um carro.\n"
msgVazios:         .asciiz "Vazios: "
msgExcluir: .asciiz "Digite o nome do morador a excluir: "
msgOcupados:        .asciiz "\nOcupados: "
msgSalvar: .asciiz "\nO conteudo foi salvo com sucesso!"
msgRecarregar: .asciiz "\nO conteudo salvo foi recarregado com sucesso!"
...
procuraEnter:                    #troca o enter do input por fim da string (\0) para que apenas a parte da string em si lida
    ...
    beqz $t2, exit               #se o byte for nulo, entra em exit para comparar com os comandos cadastrados
...
fimDeString:
    ...
    j exit            #entra em exit

exit:                       #compara com o comando exit
    la $a0, input           #carrega o endereço do input em a0
    la $a1, cmdExit         #carrega o endereço do comando "exit" em a1
    jal strcmp              #chama a função strcmp 
    bnez $v0, addAuto       #se não for "exit", verifica se é print
...
addAuto:
	la $a0, input				#carrega o endereço do input em a0
	la $a1, cmdAddAuto			#carrega o endereço do comando "cmdAddAuto" em a1
	jal strcmp					#chama a função strcmp 
	bnez $v0, infoGeral	        #se não for "cmdAddAuto", verifica se é "cmdInfoGeral"

    # Solicita o andar
    printString(msgAndar)
    ...
    printString(msgApto)      # solicita o número do apartamento
    ...
    printString(msgVeiculo)          #printa mensagem pedindo o número relativo ao tipo de carro
	...
	vagas_esgotadas:    
    	...
    	printString(msgVagasOcupadas)   #macro que printa a mensagem de vagas já ocupadas
    	j printBanner                   #volta pro loop principal

	cadastrar_moto:
    
    	# Se o endereço estiver livre vem para o cadastro da moto
    	printString(msgModelo)   #printa mensagem pedindo o modelo da moto
    	...
    	printString(msgPlaca) #printa mensagem pedindo a placa da moto
    	...
    	printString(msgMotoRegistrada)  #printa mensagem de cadastro realizado com sucesso           

    	j printBanner #volta pro loop
    
    #lógica se o tipo informado foi 1 (carro)
    verifica_carro: 
    	...
    	printString(msgModelo)  #print msg pedindo o modelo do carro
    	...
    	printString(msgPlaca)   #printa mensagem pedindo para digitar a placa
    	...
    	printString(msgCarroRegistrado) #printa a mensagem de carro foi registrado com sucesso
	...
	carro_existente: #cai aqui se já houver um carro
    	printString(msgCarroExistente)  #printa mensagem de que já existe um carro naquela vaga
    	j printBanner                   # Volta ao início
 
infoGeral: 
  # Verifica se o comando é "cmdInfoGeral"
    la $a0, input           # Carrega o endereço do input em $a0
    la $a1, cmdInfoGeral    # Carrega o endereço do comando "cmdInfoGeral" em $a1
    jal strcmp              # Chama a função strcmp
    bnez $v0, limparAp      # Se não for "cmdInfoGeral", entra em limparAp
    ...
    printString(msgVazios)    #imprime mensagem de apts vazios
    ...
    printString msgOcupados		#printa mensagem de porcentagem de aps ocupados
...
limparAp:
	...
	la $a0, input           #coloca
	la $a1, cmdLimparAp     #coloca as strings nos registradores certos para a função strcmp
	jal strcmp              #Compara o comando pra saber se é o comando "cmdLimparAp"
	bnez $v0, salvar        #Se não for, pula para salvar

    printString (msgAndar) # Pede o andar do novo morador
    ...
    printString(msgApto) # Pede o apartamento do novo morador
...
salvar:
	# verifica se o comando é cmdSalvar
	la $a0, input               # Carrega o endereço do input em $a0
	la $a1, cmdSalvar           # Carrega o endereço do comando "cmdSalvar" em $a1
	jal strcmp                  # Chama a função strcmp
	bnez $v0, recarregar        # Se não for "cmdSalvar", entra em addMorador
	...
	printString(msgSalvar)
	j printBanner

recarregar:
	# verifica se o comando é cmdRecarregar
	la $a0, input           # Carrega o endereço do input em $a0
	la $a1, cmdRecarregar   # Carrega o endereço do comando "cmdRecarregar" em $a1
	jal strcmp              # Chama a função strcmp
	bnez $v0, formatar      # Se não for "cmdRecarregar", entra em formatar
...
formatar:
	# verifica se o comando é cmdFormatar
	la $a0, input         # Carrega o endereço do input em $a0
	la $a1, cmdFormatar   # Carrega o endereço do comando "cmdFormatar" em $a1
	jal strcmp            # Chama a função strcmp
	bnez $v0, infoAp      # Se não for "cmdFormatar", entra em infoAp
	...
	printString(msgFormatar)
	j printBanner

infoAp:
	la $a0, input         #carrega o que está no endereço de input
	la $a1, cmdInfoAp     #coloca as strings nos registradores certos para a função strcmp
	jal strcmp            #Compara o comando pra saber se é o comando "cmdInfoAp"
	bnez $v0, rmvMorador  #Se não for, pula para rmvMorador


    printString msgAndar  #printa o que está em msgAndar
	...
	printAptoVazio:
   		printString msgAptoVazio     #printa mensagem de apartamento vazio
    	j comparaFinal               #entra em comparaFinal

	#imprime moradores
	printMsgMoradores:
    	printString msgMoradores      #imprime o que está em msgMoradores
...
rmvMorador:
    la $a0, input             # Carrega o endereço do input em $a0
    la $a1, cmdRmvMorador     # Carrega o endereço do comando "cmdRmvMorador" em $a1
    jal strcmp                # Chama a função strcmp
    bnez $v0, addMorador      # Se não for "cmdRmvMorador", entra em addMorador
    ...
    printString(msgExcluir)   # Pede o nome do morador a excluir
...
addMorador:
    # Verifica se o comando é "cmdAddMorador"
    la $a0, input                    # Carrega o endereço do input em $a0
    la $a1, cmdAddMorador            # Carrega o endereço do comando "cmdAddMorador" em $a1
    jal strcmp                       # Chama a função strcmp
    bnez $v0, printComandoInvalido   # Se não for "cmdAddMorador", entra em printComandoInvalido
	...
	check_moradores:
    	...
    	printString(msgAptoCheio)  #printa mensagem de apartamento cheio
	...
	registrar:
    	...
    	printString msgMoradorRegistrado    
...
copia:                         
    ...
    beq $t2, $zero, liberaPilha      # Se o byte é nulo, fim da cópia
    add $s0, $s0, 1                  # Incrementa $s0 para o próximo byte
    j copia                          # Repete o processo
